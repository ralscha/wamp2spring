...
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT, classes = CallKwTest.Config.class)
...
public class CallKwTest extends BaseWampTest {
	...
	public void testReturnValue() throws Exception {
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(1L, "callService.sum", Maps.map("a", 3).map("b", 4).getMap()), DataFormat.SMILE);
	...
	public void testDifferentProcedureName() throws Exception {
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(2L, "sum2", Maps.map("a", 11).map("b", 22).getMap()), DataFormat.SMILE);
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
		ResultMessage result = (ResultMessage) receivedMessage;
		...
		receivedMessage = sendWampMessage(new CallMessage(3L, "sum3", Maps.map("a", 4).map("b", 5).getMap()));
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
	...
	public void testNoReturnValue() throws Exception {
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(4L, "callService.noReturn", Maps.map("arg1", "name").map("arg2", 23).getMap()),
				DataFormat.CBOR);
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
		ResultMessage result = (ResultMessage) receivedMessage;
	...
	public void testWrongNumberOfArguments() throws Exception {
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(44L, "callService.noReturn", Maps.map("arg1", "name").getMap()), DataFormat.JSON);
	...
	public void testDto() throws Exception {
		...
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(8L, "callService.callWithDto", Maps.map("testDto", dto).getMap()), DataFormat.CBOR);
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
		ResultMessage result = (ResultMessage) receivedMessage;
	...
	public void testDtoAndCallMessageArgument() throws Exception {
		...
		WampMessage receivedMessage = sendWampMessage(
				new CallMessage(9L, "callService.callWithDtoAndMessage",
						Maps.map("testDto", dto).map("secondArgument", "the_second_argument").getMap()),
				DataFormat.MSGPACK);
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
		ResultMessage result = (ResultMessage) receivedMessage;
	...
	public void testArrayAndMapArgument() throws Exception {
		TestDto dto = new TestDto(2, "Hi");
		WampMessage receivedMessage = sendWampMessage(new CallMessage(9L, "callService.callWithDtoAndMessage",
				Collections.singletonList(dto), Maps.map("secondArgument", "the_second_argument").getMap(), false),
				DataFormat.CBOR);
		assertThat(receivedMessage).isInstanceOf(ResultMessage.class);
		ResultMessage result = (ResultMessage) receivedMessage;
		assertThat(result.getRequestId()).isEqualTo(9L);
