...
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT, classes = ClientToClientTest.Config.class)
@TestPropertySource(properties = "spring.main.web-application-type=reactive")
public class ClientToClientTest extends BaseWampTest {

	@Test
	public void testNoArgs() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.CBOR)) {

			wc1.connect(wampEndpointUrl());
			wc2.connect(wampEndpointUrl());

			SubscribeMessage subscribeMessage = new SubscribeMessage(1, "topic");
			SubscribedMessage subscribedMessage = wc1.sendMessageWithResult(subscribeMessage);
			wc2.sendMessageWithResult(subscribeMessage);

			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").build();
	...
	public void testOneArgs() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("the argument").build();
			wc2.sendMessage(publishMessage);
	...
	public void testMultipleArgs() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("one")
				.addArgument("two")
				.addArgument("three")
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage.getPublisher()).isNull();
			assertThat(eventMessage.isRetained()).isFalse();
			assertThat(eventMessage.getArguments()).containsExactly("one", "two", "three");
			assertThat(eventMessage.getArgumentsKw()).isNull();
	...
	public void testArgsKw() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("one", 1)
				.addArgument("two", 2)
				.addArgument("three", 3)
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage.getArgumentsKw()).containsOnly(MapEntry.entry("one", 1), MapEntry.entry("two", 2),
	...
	public void testNotExcludeMe() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			EventMessage eventMessage = (EventMessage) wc1.getWampMessage();
			assertThat(eventMessage.getSubscriptionId()).isEqualTo(subscribedMessage1.getSubscriptionId());
			assertThat(eventMessage.getPublicationId()).isGreaterThan(0L);
			assertThat(eventMessage.getTopic()).isNull();
			assertThat(eventMessage.getPublisher()).isNull();
			assertThat(eventMessage.isRetained()).isFalse();
			assertThat(eventMessage.getArguments()).isEmpty();
			assertThat(eventMessage.getArgumentsKw()).containsOnly(MapEntry.entry("one", 1));
	...
	public void testDiscloseMe() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("here")
				.discloseMe()
				.build();
			wc2.sendMessage(publishMessage);
	...
	public void testAcknowledge() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			assertThat(publishedMessage.getPublicationId()).isEqualTo(eventMessage.getPublicationId());
	...
	public void testExclude() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON);
			...
			SubscribedMessage subscribedMessage3 = wc3.sendMessageWithResult(subscribeMessage);
			...
			assertThat(eventMessage3.getSubscriptionId()).isEqualTo(subscribedMessage3.getSubscriptionId());
			...
			publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("three")
				.addExclude(wc1.getWampSessionId())
				.addExclude(wc3.getWampSessionId())
				.build();
			wc4.sendMessage(publishMessage);
	...
	public void testEligible() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON);
			...
			PublishMessage publishMessage = new PublishMessage.Builder(1L, "topic").addArgument("four")
				.addEligible(wc1.getWampSessionId())
				.addEligible(wc3.getWampSessionId())
				.build();
			wc4.sendMessage(publishMessage);
			...
			EventMessage eventMessage3 = (EventMessage) wc3.getWampMessage();
			assertThat(eventMessage3.getSubscriptionId()).isEqualTo(subscribedMessage1.getSubscriptionId());
			assertThat(eventMessage3.getPublicationId()).isGreaterThan(0L);
			assertThat(eventMessage3.getTopic()).isNull();
			assertThat(eventMessage3.getPublisher()).isNull();
			assertThat(eventMessage3.isRetained()).isFalse();
	...
	public void testPrefix() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			SubscribeMessage subscribeMessage = new SubscribeMessage(1, "news", MatchPolicy.PREFIX);
			SubscribedMessage subscribedMessage1 = wc1.sendMessageWithResult(subscribeMessage);
			wc2.sendMessageWithResult(subscribeMessage);

			PublishMessage publishMessage = new PublishMessage.Builder(1L, "news.world").addArgument("test")
				.addArgument("test", 1012)
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage1.getArgumentsKw()).containsOnly(MapEntry.entry("test", 1012));

			wc2.waitForNothing();

			publishMessage = new PublishMessage.Builder(1L, "news.business").addArgument("company")
				.addArgument("test", 1013)
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage1.getArgumentsKw()).containsOnly(MapEntry.entry("test", 1013));

			wc2.waitForNothing();

			publishMessage = new PublishMessage.Builder(1L, "newArticle").addArgument("test", 1014).build();
			wc2.sendMessage(publishMessage);
	...
	public void testWildcard() throws Exception {
		try (WampClient wc1 = new WampClient(DataFormat.JSON); WampClient wc2 = new WampClient(DataFormat.MSGPACK)) {
			...
			SubscribeMessage subscribeMessage = new SubscribeMessage(1, "crud..create", MatchPolicy.WILDCARD);
			SubscribedMessage subscribedMessage1 = wc1.sendMessageWithResult(subscribeMessage);
			SubscribedMessage subscribedMessage2 = wc2.sendMessageWithResult(subscribeMessage);

			PublishMessage publishMessage = new PublishMessage.Builder(1L, "crud.user.create").addArgument("id", 11)
				.addArgument("username", "joe")
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage1.getArgumentsKw()).containsOnly(MapEntry.entry("id", 11),
					MapEntry.entry("username", "joe"));

			wc2.waitForNothing();

			publishMessage = new PublishMessage.Builder(1L, "crud.company.create").addArgument("id", 12)
				.addArgument("name", "solar inc.")
				.notExcludeMe()
				.build();
			wc2.sendMessage(publishMessage);
			...
			assertThat(eventMessage1.getArguments()).isEmpty();
			assertThat(eventMessage1.isRetained()).isFalse();
			assertThat(eventMessage1.getArgumentsKw()).containsOnly(MapEntry.entry("id", 12),
					MapEntry.entry("name", "solar inc."));
			...
			assertThat(eventMessage2.isRetained()).isFalse();
			assertThat(eventMessage2.getArgumentsKw()).containsOnly(MapEntry.entry("id", 12),
					MapEntry.entry("name", "solar inc."));

			publishMessage = new PublishMessage.Builder(1L, "crud.create").addArgument("id", 13).build();
			wc2.sendMessage(publishMessage);
