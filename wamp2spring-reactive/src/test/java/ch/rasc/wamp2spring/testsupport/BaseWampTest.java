...
public class BaseWampTest {
	...
	protected WebSocketHttpHeaders getHeaders(DataFormat dataFormat) {
		...
		if (dataFormat == DataFormat.MSGPACK) {
			headers.setSecWebSocketProtocol(WampWebSocketHandler.MSGPACK_PROTOCOL);
		}
		else if (dataFormat == DataFormat.SMILE) {
			headers.setSecWebSocketProtocol(WampWebSocketHandler.SMILE_PROTOCOL);
		}
		else if (dataFormat == DataFormat.CBOR) {
			headers.setSecWebSocketProtocol(WampWebSocketHandler.CBOR_PROTOCOL);
		}
		else {
			headers.setSecWebSocketProtocol(WampWebSocketHandler.JSON_PROTOCOL);
		}
		return headers;
	}
