...
@Configuration
public class WampReactiveConfiguration implements ImportAware {

	@Nullable
	private ConversionService internalConversionService;

	private final List<WampReactiveConfigurer> configurers = new ArrayList<>();

	private final Features features = new Features();

	@Autowired(required = false)
	public void setConfigurers(List<WampReactiveConfigurer> configurers) {
		if (!CollectionUtils.isEmpty(configurers)) {
			this.configurers.addAll(configurers);

			configureFeatures(this.features);
			for (WampReactiveConfigurer wc : this.configurers) {
	...
	public void setImportMetadata(AnnotationMetadata importMetadata) {
		Map<String, Object> attributes = AnnotationAttributes.fromMap(importMetadata
				.getAnnotationAttributes(EnableReactiveWamp.class.getName(), false));
		if (attributes != null) {
	...
	public HandlerMapping handlerMapping() {
		...
		for (WampReactiveConfigurer wc : this.configurers) {
			wc.configureHandlerMapping(mapping);
		}
