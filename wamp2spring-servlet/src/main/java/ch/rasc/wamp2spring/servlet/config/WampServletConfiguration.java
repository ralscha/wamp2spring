...
@Configuration
public class WampServletConfiguration implements ImportAware {
	...
	private final List<WampServletConfigurer> configurers = new ArrayList<>();

	private final Features features = new Features();

	@Autowired(required = false)
	public void setConfigurers(List<WampServletConfigurer> configurers) {
		if (!CollectionUtils.isEmpty(configurers)) {
			this.configurers.addAll(configurers);

			configureFeatures(this.features);
			for (WampServletConfigurer wc : this.configurers) {
	...
	public void setImportMetadata(AnnotationMetadata importMetadata) {
		Map<String, Object> attributes = AnnotationAttributes.fromMap(importMetadata
				.getAnnotationAttributes(EnableServletWamp.class.getName(), false));
		if (attributes != null) {
